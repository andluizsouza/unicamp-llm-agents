...
description: >-
  ...
  escrever ou alterar .py: PEP, docstrings, SOLID, modularização, refactor e
  fronteiras (tools, grafos, RAG, MCP, CLI).
---
...
1. `git diff` (ou arquivos apontados). Foque no pacote do app e em `eval/`.
...
3. Checklist:
   ...
   - nenhuma lógica de agente/registry/métrica em `.ipynb`
4. Se o ambiente existir, rode `make lint` (venv ativo).

## Feedback

- 🔴 Critical — bugs, segredo vazado, nó god-object, implementação do sistema em notebook
- 🟡 Should fix — SOLID, nomes, tipos faltando
- 🟢 Nice — clareza
